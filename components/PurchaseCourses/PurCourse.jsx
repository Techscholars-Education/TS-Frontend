  ...
  return (
    <div className="bg-[#f7faff]  font-Poppins h-screen w-full overflow-x-hidden">
      ...
      {loading ? (
        ...
        <div className="md:my-6 flex flex-col  h-[82vh] md:max-w-[80vw] ">
         <div>
         <h2 className="text-xl md:text-xl font-semibold text-darkBlue mx-4">
          ...
          <div className="md:max-w-[82vw] w-[90vw]  rounded-md flex flex-col pb-3">
            <div className="grid grid-cols-1 md:grid-cols-2 md:gap-3  lg:grid-cols-3 gap-4 mt-4 xl:w-10/12 md:mx-auto">
              {data.map((item) => (
                <Card
                  key={item.id}
                  title={item.title}
